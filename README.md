...
* [`spark`](https://github.com/apache/spark) ⭐ 43,981 | 🐛 546 | 🌐 Scala | 📅 2026-09-11: the de-facto leader of distributed computing, now it is faring more competition. It offers [`MLib`](https://spark.apache.org/mllib/), a machine learning library, as well as [`GraphX`](https://spark.apache.org/graphx/) for "graphs and graph-parallel computation". Its Python API is [`pyspark`](https://github.com/apache/spark/tree/master/python) ⭐ 43,981 | 🐛 546 | 🌐 Scala | 📅 2026-09-11.
* [`dask`](https://github.com/dask/dask) ⭐ 13,915 | 🐛 1,330 | 🌐 Python | 📅 2026-08-24 is among the first distributed computing DataFrame libraries, alongside `spark`. `dask` does not only offer a [distributed computing equivalent](https://dask.org/) of `pandas`, but also of [`numpy`](https://github.com/numpy/numpy) ⭐ 32,723 | 🐛 2,296 | 🌐 Python | 📅 2026-09-11 and [`scikit-learn`](https://github.com/scikit-learn/scikit-learn) ⭐ 67,230 | 🐛 2,151 | 🌐 Python | 📅 2026-09-11, so that "you don't have to completely rewrite your code or retrain to scale up", i.e. to run code on distributed clusters. A Dask DataFrame "is a large parallel DataFrame composed of many smaller Pandas DataFrames, split along the index".
...
* [`arrow-datafusion`](https://github.com/apache/arrow-datafusion) ⭐ 9,306 | 🐛 2,186 | 🌐 Rust | 📅 2026-09-11: this is [Arrow's query engine](https://arrow.apache.org/datafusion/user-guide/introduction.html), written in the Rust programming language. Much like `duckdb`, `datafusion` offers both a SQL and Python-like API interface. Much like `pyspark`, `datafusion` "allows you to build a plan through SQL or a DataFrame API against in-memory data, parquet or CSV files, run it in a multi-threaded environment, and obtain the result back in Python".
  * [`ballista`](https://github.com/apache/arrow-datafusion/blob/master/ballista/README.md) ⭐ 9,306 | 🐛 2,186 | 🌐 Rust | 📅 2026-09-11 is the distributed query engine, written in Rust, built on top of `arrow-datafusion`. [Here](https://arrow.apache.org/datafusion/user-guide/distributed/introduction.html#how-does-this-compare-to-apache-spark) is a comparison between `ballista` and `spark`.
...
* [`pydatatable`](https://github.com/h2oai/datatable) ⭐ 1,876 | 🐛 183 | 🌐 C++ | 📅 2026-09-01 is a Python port of the astounding [`{data.table}`](https://github.com/Rdatatable/data.table) ⭐ 3,917 | 🐛 984 | 🌐 R | 📅 2026-09-10 library in R, that achieves impressive results thanks to parallelisation.
* [`pyjanitor`](https://github.com/pyjanitor-devs/pyjanitor) ⭐ 1,499 | 🐛 158 | 🌐 Python | 📅 2026-09-08 was originally conceived as a `pandas` extension of the well-known `{janitor}` R package. The latter was a package to clean strings with ease in a R'`data.frame` or `tibble` objects, but later incorporated new methods to make it more similar to `{dplyr}`. Adding and removing column, for example, is easier with the dedicated methods `df.remove_column()` and `df.add_column()`, but also renaming column is easier with `df.rename_column()`. This enables to run smoother pipelines that exploit *method chaining*.
